...
from models.modules import ApproachNet, GraspableNet, CloudCrop, SWADNet, Exactor, CoGuided_Fusion_Conv
from loss_utils import GRASP_MAX_WIDTH, NUM_VIEW, NUM_ANGLE, NUM_DEPTH, GRASPNESS_THRESHOLD, M_POINT, ATTENS_THREAD
...
class GraspNet(nn.Module):
    def __init__(self, cylinder_radius=0.05, seed_feat_dim=512, is_training=True):
        ...
        self.image_feat = Exactor()
        self.fusion = CoGuided_Fusion_Conv(inplanes_I=128, inplanes_P=512, outplanes=512)
    ...
    def forward(self, end_points):
        seed_xyz = end_points['point_clouds']  # use all sampled point cloud, B*Ns*3  （2, 15000, 3）
        color = end_points['color']
        ...
        color = self.image_feat(color)
        color = color.view(B, 128, -1)
        xyz_index = end_points['xyz_index'].unsqueeze(1).repeat(1, 128, 1)
        color = torch.gather(color, 2, xyz_index)

        seed_features, atten = self.fusion(seed_features, color)
        attens = atten.cpu().detach().numpy()
        attens = torch.from_numpy(attens).cuda()
        ...
        graspable_mask = objectness_mask
        ...
        for i in range(B):
            ...
            cur_atten = attens[i].transpose(1, 0)
            cur_atten = cur_atten[cur_mask]
            cur_atten_flipped = cur_atten.unsqueeze(0).transpose(1, 2).contiguous()
            ...
            fps_idxs = fps_idxs.unsqueeze(0).long()
            cur_atten_ = torch.gather(cur_atten_flipped, 2, fps_idxs).squeeze(0).squeeze(0)
            cur_atten_mask = (cur_atten_ > ATTENS_THREAD)

            cur_feat = cur_feat.transpose(1, 0)
            cur_feat = cur_feat[cur_atten_mask].transpose(1, 0)
            cur_seed_xyz = cur_seed_xyz[cur_atten_mask]

            cur_feat = cur_feat[:, :1024]
            cur_seed_xyz = cur_seed_xyz[:1024, :]

            seed_features_graspable.append(cur_feat)
            seed_xyz_graspable.append(cur_seed_xyz)
        seed_xyz_graspable = torch.stack(seed_xyz_graspable, 0)  # B*Ns*3  (2, 1024, 3)
        seed_features_graspable = torch.stack(seed_features_graspable)  # B*feat_dim*Ns (2, 512, 1024)
...
def pred_decode(end_points):
    ...
    for i in range(batch_size):
        ...
        grasp_score = grasp_score.view(1024, NUM_ANGLE*NUM_DEPTH)
