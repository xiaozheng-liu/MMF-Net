...
def get_loss(end_points):
    ...
    loss = objectness_loss + 100 * view_loss + 15 * score_loss + 10 * width_loss
...
def compute_view_graspness_loss(end_points):
    ...
    view_score = end_points['view_score']  # (2, 1024, 300)
    view_label = end_points['batch_grasp_view_graspness']  # (2, 1024, 300)
...
def compute_score_loss(end_points):
    criterion = nn.SmoothL1Loss(reduction='mean')
    grasp_score_pred = end_points['grasp_score_pred']  # (2, 1024 ,12 ,4)
    grasp_score_label = end_points['batch_grasp_score']  # (2, 1024 ,12 ,4)
...
def compute_width_loss(end_points):
    criterion = nn.SmoothL1Loss(reduction='none')
    grasp_width_pred = end_points['grasp_width_pred']  # (2, 1024 ,12 ,4)
    grasp_width_label = end_points['batch_grasp_width'] * 10  # (2, 1024 ,12 ,4)
