...
class GraspNetDataset(Dataset):
    ...
    def get_data(self, index, return_raw_cloud=False):
        color = cv2.imread(self.colorpath[index], cv2.IMREAD_COLOR)
        color = cv2.cvtColor(color, cv2.COLOR_BGR2RGB)  # covert the image channel
        color = cv2.resize(color, dsize=(640, 360), interpolation=cv2.INTER_LINEAR)
        color = np.float32(color) / 255.0
        ...
        xyz = np.arange(921600).reshape(720, 1280)
    ...
    def get_data_label(self, index):
        ...
        depth = np.array(Image.open(self.depthpath[index]))  # ndarrary (720, 1280) int32
        seg = np.array(Image.open(self.labelpath[index]))
        meta = scio.loadmat(self.metapath[index])
        scene = self.scenename[index]
        try:
        ...
        # generate cloud
        cloud = create_point_cloud_from_depth_image(depth, camera, organized=True)  # (720,1280, 3) float 64
        xyz = np.arange(921600).reshape(720, 1280)  # （720, 1280） int64
        ...
        if self.remove_outlier:
            ...
            trans = np.dot(align_mat, camera_poses[self.frameid[index]])
            workspace_mask = get_workspace_mask(cloud, seg, trans=trans, organized=True, outlier=0.02)
            mask = (depth_mask & workspace_mask)  # (720, 1280) bool
        else:
            mask = depth_mask
        cloud_masked = cloud[mask]
        seg_masked = seg[mask]
...
def load_grasp_labels(root):
    ...
    for obj_name in tqdm(obj_names, desc='Loading grasping labels...'):
        label = np.load(
            os.path.join(root, 'grasp_label_simplified', '{}_labels.npz'.format(str(obj_name - 1).zfill(3))))
